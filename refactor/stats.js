...
function STATS() {
  ...
  this.tooltip  = { };
  // tooltips
  this.tooltip.n = 'The number of items within the set or distribution.';
  ...
  this.tooltip.quartile = 'If a set of numbers is sorted from the smallest value to the largest value, then the number that occurs midway through the set is the second quartile (also called the median).  The number that occurs midway between the smallest value and the median is the first quartile, and the number that occurs midway between the median and the largest value is the third quartile.  When viewing a set of numbers graphically as a distribution, a large portion of the numbers will tend to fall between the first and third quartiles.';
  ...
  this.load = function(data) {
    ...
    new_data = this.sort(new_data);
		for (let i = new_data.length - 1; i >= 0; i--) {
			if (typeof (new_data[i]) !== 'number') { new_data.splice(i, 1); }
		}
    this.data = new_data;
    this.value = { };
  }
