...
function PROJECT() {
	...
	this.set_directory = async (path) => {
		...
		this.path_records.ortho_cds_aln = await path_record.clone();
		...
		await this.path_records.ortho_cds_aln.add_folder('ortho_cds_aln');
		...
		this.folders.ortho_cds_aln = await this.path_records.ortho_cds_aln.get_full_path();
